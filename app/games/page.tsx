...
export default async function Games({
  ...
  let games = null;
  let competitionLogosById = new Map<string, string | null>();
  let competitionLogosByName = new Map<string, string | null>();

  try {
    const [gamesResponse, competitions] = await Promise.all([
      getGames(currentPage, startDate, endDate),
      getAllCompetitions(),
    ]);

    games = gamesResponse;

    competitionLogosById = new Map(
      competitions.map((competition) => [competition.id, competition.logoUrl]),
    );

    competitionLogosByName = new Map(
            ...
            <Card key={game.id} className={surfaceCardClassName}>
              <CardHeader className="space-y-3">
                <div className="flex items-start justify-between gap-2">
                  {game.competition ? (
                    <div className="flex min-w-0 items-start gap-2 text-xs font-medium uppercase tracking-[0.22em] text-primary/70">
                      <CompetitionLogo
                        competitionName={game.competition}
